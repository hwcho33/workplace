      ...
      <div class="form-wrap">
        <div class="find-tab-wrap">
          <ul>
            ...
            <li :class="{ active: !isPhone }">
              <div class="tab-btn">
                <a href="javascript:void(0)" class="tab-btn" @click="_onClickToByEmail">
...
import { postPasswordSearchSend, getPasswordSearchVerification } from '~/api/customer'

///////////////////////////////////////////////////////////////////
// 3. 타입스크립트 관련 모듈 (types, enums, interface, class)
///////////////////////////////////////////////////////////////////
import { PasswordSearchSendResponse, SearchType } from '~/api/types/customer'
...
///////////////////////////////////////////////////////////////////
onBeforeMount(() => {
  presetId = router.options.history.state?.presetId as string

  // 아이디가 없을 경우 잘못된 접근으로 간주하고 404 페이지로 이동
  if (!presetId) {
...
// PASSWORD 찾기 인증번호 전송 api 호출
async function callPasswordSearchSend(): Promise<void> {
  const { data } = await postPasswordSearchSend({
...
// 전송 완료
function sendResult7806(data: PasswordSearchSendResponse): void {
  // 타이머 시간 설정. -1초는 싱크를 위해
  timerSec.value = Number(data.vaildTm) - 1
  certSeq = Number(data.seqNum)
...
async function _onClickVerification(): Promise<void> {
  ...
  // API 호출
  callPasswordSearchVerification()
}

// Password 인증 확인 API 호출
async function callPasswordSearchVerification(): Promise<void> {
  const { data } = await getPasswordSearchVerification({
    loginId: String(presetId),
...
// 인증번호 확인(성공)
function verifResult7809(): void {
  // 비밀번호 변경으로 이동
  router.replace({
    path: '/customer/login/password-change',
    state: { presetId }
  })
}
