<template>
  <div class="password-change-wrap base-change">
    ...
    <div class="inner-wrap">
      <div class="input-form-wrap">
        ...
        >
          <label for="userPassword">{{ $t('label.customer.0080', '새로운 비밀번호를 입력해 주세요.') }}</label>
        ...
        >
          ...
          <div class="button-group-wrap">
            <button type="button" class="certify-btn" @click="_onClickConfirm">
...
async function callPasswordChange(): Promise<void> {
  const { data } = await putPasswordChange({ loginId: String(presetId), paramComb: userPassword.value.value })
...
function changeResult7951(): void {
  ...
  // 로그인 페이지로 전환
  router.replace({ path: '/customer/login', state: { presetId } })
}
